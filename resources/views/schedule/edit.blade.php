...
<div class="flex flex-col items-center">
  <h2 class="text-black text-3xl font-medium ">Update Schedule</h2>
  <form action="{{ route('schedule.update', $schedule->id) }}" class="w-1/3 mt-16" method="POST">
    @csrf
    @method('PATCH')
    <div class="mb-6">
      ...
      <select name="movie_id" id="movie_id"
        ...
        <option value="{{ $movie->id }}" @if ($movie->id == $schedule->movie_id)
          selected
    ...
    <div class="mb-6">
      ...
      <select name="studio_id" id="studio_id"
        ...
        <option value="{{ $studio->id }}" @if ($studio->id == $schedule->studio_id)
          selected
          @endif>{{ $studio->name }}</option>
        @endforeach
      </select>
    </div>
    <div class="mb-6">
